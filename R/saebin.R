...
saebincov<- function(y,non_y,n,x,N,area,unit,data){
  result <- list(saebincov = NA, fit = list(estcoef
                                            = NA, call = NA), randeff = NA)
  xfrom <- x
  ...
  xbeta.unit.s <- summarise(group_by(sample, unit), area,n,y,N)
  xbeta.unit.s <- cbind(xbeta.unit.s,xbeta.s)
  #nilai XB+reffar tiap area
  xbeta.reff <- left_join(xbeta.unit.s,area.reff,by="area")
  xbeta.reff$nilai <- xbeta.reff$xbeta+xbeta.reff$randeff
  ...
  xbeta.reff$pij <- exp(xbeta.reff$nilai)/(1+exp(xbeta.reff$nilai))
  ...
  yi.sample <- summarise(group_by(xbeta.reff, area), jumpop=sum(N), yi = sum(N*pij))
  yi.non_sample <- summarise(group_by(xbeta.unit.ns, area), jumpop=sum(N), yi = sum(N*pij))
  yi.area <- rbind(yi.sample,yi.non_sample)
  pi.area <- summarise(group_by(yi.area, area),pi =sum(yi)/sum(jumpop) )

  result$saebincov <- pi.area
  result$fit$estcoef <- estcoef
  result$fit$call <- model
